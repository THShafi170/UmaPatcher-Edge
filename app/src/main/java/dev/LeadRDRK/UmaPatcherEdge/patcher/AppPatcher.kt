...
import dev.LeadRDRK.UmaPatcherEdge.core.GitHubReleases
...
import dev.LeadRDRK.UmaPatcherEdge.shizuku.ShizukuInstaller
import dev.LeadRDRK.UmaPatcherEdge.shizuku.ShizukuState
import dev.LeadRDRK.UmaPatcherEdge.utils.bytesToHex
...
class AppPatcher(
    ...
    private suspend fun extractZipWithProgress(context: Context, zip: ZipExtractor, extractDir: File) {
        ...
        extractDir.deleteRecursively()
        zip.extractAll(extractDir, { ensureActive() }) { progress = it }
    }
    ...
    private suspend fun syncModLibs(context: Context): String? {
        ...
        val modSource = context.getPrefValue(PrefKey.HACHIMI_MOD_SOURCE) as String
        if (modSource == "local") {
            ...
            try {
                ...
                context.contentResolver.openInputStream(customUri).use { input ->
                    if (input == null) throw IOException("Failed to open local library input stream")
                    val arm64Lib = context.modArm64Lib
                    arm64Lib.outputStream().use { output ->
                        input.copyTo(output)
                    }
                }
                val detectedVer = getHachimiVersionFromSo(context.modArm64Lib) ?: "local"
